...
#     H[1][1] = w_2
#     H[2][2] = w_3
#     H[0][1] = -Omega_c * 2 * np.cos(w_c * t) -Omega_p*2*np.cos(w_p*t)
#     H[1][0] = H[0][1]
#     H[0][2] = H[0][1]
#     H[2][0] = H[0][2]
#     H *= -1j
...
def L_lambda(
    ...
    L_lambda_dissipative(
        L_sys_int, gamma_12=gamma_12, gamma_13=gamma_13, n_12=n_12, n_13=n_13
    )

    return L_sys_int


def H_three_state_RWA(
    t,
    w_c=1,
    w_1=2,
    w_2=1,
...
def U_RWA_closed(
    t, w_c=1, w_1=2, w_2=1, w_p=2, w_3=0, Omega_c=0.56, Omega_p=0.50
):  # Only good at TPR!
...
def L_lambda_dissipative(L_sys_int, gamma_12=0.9, gamma_13=1, n_12=0, n_13=0):
    a = gamma_12 * (1 + n_12)
    b = gamma_12 * n_12
    c = gamma_13 * (1 + n_13)
    d = gamma_13 * n_13
...
def M_14_M_23_M_32_M_41(points, dt):
    ...
    M_23 = dt**2 / 6720 * (
        comm(
            232 / 39 * points[0] + 1152 / 13 * points[2] + 72 * points[4],
    ...
    M_32 = (
        ...
        * (
            64
            * (
                comm(points[3] + points[6], comm(points[3], points[0]))
                + comm(points[3] + points[0], comm(points[3], points[6]))
            )
            + 44
            * (
                comm(points[0], comm(points[0], points[3]))
    ...
    M_41 = (
        ...
        / 5040
        * comm(
            1 / c * points[0] - points[6],
            comm(points[6] - c * points[0], comm(points[6], points[0])),
        )
    )
    # M_41=0
...
def M_12_M_21(points, dt):
    ...
    M_21 = 0.5 * dt**2 / 6 * comm(points[2], points[0])
    # M_2 = 0
...
def propagate(H_name, method_name, save_name, H_parameters, times, mapping=False):
    H_func, method_func, save_func = get_funcs_from_names(
        H_name, method_name, save_name
    )
    dt = times[1] - times[0]
    # print(dt)
    if mapping == False:
        if method_func[0] == "save_point":
            return propagate_save_point(
                H_func, method_func, save_func, H_parameters["np"], dt, times
            )
    if mapping == True:
        if method_func[0] == "save_point":
            return propagate_save_point_mapping(
                H_func, method_func, save_func, H_parameters["np"], dt, times
            )


def propagate_save_point_mapping(
    H_func, method_func, save_func, H_parameters, dt, times
):
    ...
    for i in range(len(times) - 1):
        ...
        H_points[1:] = [
            H_function(point + times[i + 1], **H_parameters) for point in points[1:]
        ]
